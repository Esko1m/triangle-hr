...
const AboutSection = () => {
  ...
  // Mapping icons to titles
  const iconMap = {
    'Our Vision': Eye,
    'Our Mission': Target,
    'Our Objective': Compass,
  };
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="grid lg:grid-cols-12 gap-16 items-center mb-32">
          ...
          >
            ...
            <h2 className="text-5xl md:text-6xl font-black text-primary uppercase tracking-tighter mb-10 font-display">{about.title}</h2>
            ...
            <div className="space-y-6">
              <h3 className="text-2xl font-bold text-primary mb-6">{about.solutionsTitle}</h3>
              {about.solutions.map((item, i) => (
                <motion.div 
                  key={i} 
            ...
            <div className="relative aspect-square flex items-center justify-center">
              ...
              <div className="relative z-10 w-full max-w-[320px] aspect-square rounded-[3rem] bg-white/40 backdrop-blur-xl border border-white/40 shadow-2xl flex flex-col items-center justify-center p-8 text-center overflow-hidden">
                 <div className="w-24 h-24 bg-primary text-white rounded-2xl flex items-center justify-center mb-6 shadow-xl shadow-primary/20 rotate-12">
                   <span className="text-4xl font-black font-display -rotate-12">{about.stats.value}</span>
                 </div>
                 <h4 className="text-3xl font-black font-display text-primary leading-tight mb-2 uppercase">{about.stats.title}</h4>
                 <p className="text-foreground/70 font-bold text-sm uppercase tracking-widest">{about.stats.subtitle}</p>
          ...
          <div className="p-10 md:p-20">
            <div className="grid lg:grid-cols-4 gap-12 items-start">
              <div className="lg:col-span-1">
                 <h3 className="text-sm font-black text-primary uppercase tracking-[0.3em] mb-4">{about.framework.label}</h3>
                 <h4 className="text-4xl font-extrabold font-display leading-tight mb-6">{about.framework.title}</h4>
                 <p className="text-muted-foreground font-medium text-lg">{about.framework.description}</p>
              </div>
              
              <div className="lg:col-span-3 grid md:grid-cols-3 gap-8">
                {about.pillars.map((p, i) => {
                  const Icon = iconMap[p.title as keyof typeof iconMap] || Compass;
                  const colors = [
                  ...
                  return (
                    ...
                    >
                      <div className={`w-16 h-16 ${style.bg} ${style.color} rounded-2xl flex items-center justify-center mb-8 shadow-sm group-hover:shadow-lg transition-all duration-300`}>
                        <Icon size={32} />
                      </div>
